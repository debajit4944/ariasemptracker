...
<div class="card">
	...
	<div class="card-header">
        ...
        <div class="float-right">
            <!-- <button type="button" class="btn btn-primary"><i class="fa fa-table"></i> &nbsp;List All Offices</button> -->
            <a class="btn btn-primary" href="{{ route('office.index') }}"><i class="fa fa-table"></i> &nbsp;List All Offices</a>
        </div>
    </div>
	<div class="card-body">
        <div>
            <form action="{{ route('office.update', $office->id) }}" method="post">
                ...
                @method("PATCH")
                <label>Name</label></br>
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $office->name) }}">
                @error('name')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form method="POST" action="{{ route('office.destroy', $office->id) }}" accept-charset="UTF-8" style="display:inline">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
