...
@elseif (session('flash_message_emp_desg_update'))
    <div class="alert alert-success">
        {{ session('flash_message_emp_desg_update') }}
    </div>
@elseif (session('flash_message_emp_district_update'))
    <div class="alert alert-success">
        {{ session('flash_message_emp_district_update') }}
    </div>
@elseif (session('flash_message_emp_office_update'))
    <div class="alert alert-success">
        {{ session('flash_message_emp_office_update') }}
    </div>
@elseif (session('flash_message_emp_ctp_update'))
    <div class="alert alert-success">
        {{ session('flash_message_emp_ctp_update') }}
    </div>
@endif
...
<div class="card">
	...
	<div class="card-header">
        ...
        <div class="float-left">
            <a class="btn btn-primary" href="{{ route('employees.displayAllEmp') }}"><i class="fa fa-sitemap"></i>&nbsp; Show All Employees</a>
        </div>
        ...
        <div>
            <table id="example" class="table table-striped table-bordered" style="width:100%; font-size:0.90em;">
                ...
                @if($employees)
                    ...
                    <tr>
                        <td>
                            <a href="{{route('generate-pdf', $employee->id)}}">{{$employee->name}}</a>
                        </td>
